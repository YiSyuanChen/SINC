    ...
    for split in ["train", "val"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                if split == "train":
                    setattr(pl_module, f"{split}_{k}_main_accuracy", Accuracy())
                    setattr(pl_module, f"{split}_{k}_exp_accuracy", Accuracy())
                    setattr(pl_module, f"{split}_{k}_loss", Scalar())
                else:
                    setattr(pl_module, f"{split}_{k}_nonbursty_main_accuracy", Accuracy())
                    setattr(pl_module, f"{split}_{k}_nonbursty_loss", Scalar())
                    if not pl_module.hparams.config["no_example_baseline"]:
                        setattr(pl_module, f"{split}_{k}_nway_main_accuracy", Accuracy())
                        setattr(pl_module, f"{split}_{k}_nway_exp_accuracy", Accuracy())
                        setattr(pl_module, f"{split}_{k}_nway_loss", Scalar())
                        setattr(pl_module, f"{split}_{k}_nonbursty_exp_accuracy", Accuracy())
            else:
                setattr(pl_module, f"{split}_{k}_accuracy", Accuracy())
                setattr(pl_module, f"{split}_{k}_loss", Scalar())
...
def epoch_wrapup(pl_module):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            getattr(pl_module, f"{phase}_{loss_name}_loss").reset()
        elif loss_name == "pcls": # NEW
            if phase == "train":
                value = getattr(pl_module, f"{phase}_{loss_name}_main_accuracy").compute()
                pl_module.log(f"{loss_name}/{phase}/main_accuracy_epoch", value)
                getattr(pl_module, f"{phase}_{loss_name}_main_accuracy").reset()
                ...
                if not pl_module.hparams.config["no_example_baseline"]:
                    value_exp = getattr(pl_module, f"{phase}_{loss_name}_exp_accuracy").compute()
                    pl_module.log(f"{loss_name}/{phase}/exp_accuracy_epoch", value_exp)
                    getattr(pl_module, f"{phase}_{loss_name}_exp_accuracy").reset()
            else:
                value_1 = getattr(pl_module, f"{phase}_{loss_name}_nonbursty_main_accuracy").compute()
                pl_module.log(f"{loss_name}/{phase}/nonbursty_main_accuracy_epoch", value_1)
                getattr(pl_module, f"{phase}_{loss_name}_nonbursty_main_accuracy").reset()

                pl_module.log(
                    f"{loss_name}/{phase}/nonbursty_loss_epoch",
                    getattr(pl_module, f"{phase}_{loss_name}_nonbursty_loss").compute(),
                )
                getattr(pl_module, f"{phase}_{loss_name}_nonbursty_loss").reset()

                if not pl_module.hparams.config["no_example_baseline"]:
                    value_2 = getattr(pl_module, f"{phase}_{loss_name}_nway_main_accuracy").compute()
                    pl_module.log(f"{loss_name}/{phase}/nway_main_accuracy_epoch", value_2)
                    getattr(pl_module, f"{phase}_{loss_name}_nway_main_accuracy").reset()

                    value_exp = getattr(pl_module, f"{phase}_{loss_name}_nway_exp_accuracy").compute()
                    pl_module.log(f"{loss_name}/{phase}/nway_exp_accuracy_epoch", value_exp)
                    getattr(pl_module, f"{phase}_{loss_name}_nway_exp_accuracy").reset()

                    pl_module.log(
                        f"{loss_name}/{phase}/nway_loss_epoch",
                        getattr(pl_module, f"{phase}_{loss_name}_nway_loss").compute(),
                    )
                    getattr(pl_module, f"{phase}_{loss_name}_nway_loss").reset()

                    value_exp = getattr(pl_module, f"{phase}_{loss_name}_nonbursty_exp_accuracy").compute()
                    pl_module.log(f"{loss_name}/{phase}/nonbursty_exp_accuracy_epoch", value_exp)
                    getattr(pl_module, f"{phase}_{loss_name}_nonbursty_exp_accuracy").reset()

                if pl_module.hparams.config["meta_save_metric"] == "in_weight":
                    value = value_1
                elif pl_module.hparams.config["meta_save_metric"] == "in_context":
                    assert not pl_module.hparams.config["no_example_baseline"]
                    value = value_2
                else:
                    assert not pl_module.hparams.config["no_example_baseline"]
