...
def config():
    ...
    meta_encoder_type = "autoregressive"
    meta_encoder_size = "custom"

    # Demonstrations
    no_example_baseline = False
    example_num = 8
    bursty_ratio = 0.9
    num_eval_ways = 2
    ...
    feature_source = None
    only_vl_feats = False
    only_v_and_l_feats = False

    # Pre-training
    pcls_label_size = None
    pretrain_scale = "small"
    meta_save_metric = "in_context"
    label_multiplicity = None
    correlation_embeddings=True

    # Downstream
    select_demonstrations = True
    vqa_test_with_labels = False
...
# NEW
@ex.named_config
def task_finetune_sinc_clip_bert():
    exp_name = "finetune_sinc"
    datasets = ["sinc"]
    loss_names = _loss_names({"pcls": 1})
    batch_size = 64
    max_epoch = 10
    max_steps = None
    warmup_steps = 0.1
...
# NEW
@ex.named_config
def task_finetune_open_mi_clip_bert():
    exp_name = "finetune_open_mi"
    datasets = ["open_mi"]
    loss_names = _loss_names({"open_mi": 1})
    batch_size = 64
    max_epoch = 5
    max_steps = None
    warmup_steps = 0.1
...
# NEW
@ex.named_config
def task_finetune_fast_vqa_clip_bert():
    exp_name = "finetune_fast_vqa"
    datasets = ["fast_vqa"]
    loss_names = _loss_names({"fast_vqa": 1})
    batch_size = 64
    max_epoch = 5
    max_steps = None
    warmup_steps = 0.1
