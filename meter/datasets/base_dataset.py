...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if text_column_name != "" and not self.image_only:
            k = 0
            for i in range(len(self.table[text_column_name])):
                for j in range(len(self.table[text_column_name][i])):
        ...
        self.sample_index = np.arange(len(self.index_mapper))
        if max_samples is not None:
            np.random.seed(config["sample_seed"]) if self.split=="train" else np.random.seed(0)
            self.sample_index = np.random.choice(self.sample_index, max_samples, replace=False)

        # NEW
        # Workaround for OOM problem
        # REF: https://github.com/pytorch/pytorch/issues/13246#issuecomment-905703662
    ...
    def get_text(self, raw_index):
        ...
        # NEW
        text = self.table[self.text_column_name][index][caption_index].as_py()
        if type(text) == list: # SNLI-VE
            text = text[1]
